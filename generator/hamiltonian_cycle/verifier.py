...
def verify(pred, answer, meta):
    ...
    # Accept legacy 'NO' and standardized 'unsolvable' tokens
    if isinstance(answer, str) and answer.strip() in {"NO", "unsolvable"}:
        return 1 if (isinstance(final_answer, str) and final_answer.strip() in {"NO", "unsolvable"}) else 0
